...
namespace Giftins.Core.Infrastructure
{
    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        private static AppDomainTypeFinder _global;

        #endregion

        static AppDomainTypeFinder()
        {
            _global = new AppDomainTypeFinder();
        }
        ...
        {
            return FindTypesOfType(assignTypeFrom, assemblies, false, !onlyConcreteClasses, t => t.IsClass);
        }

        public IEnumerable<Type> FindInterfacesOfType<T>()
        {
            var tType = typeof(T);
            if (!tType.IsInterface)
                throw new ArgumentException($"Interface type required.");

            return FindInterfacesOfType(tType);
        }
        ...
        public IEnumerable<Type> FindInterfacesOfType<T>(IEnumerable<Assembly> assemblies)
        {
            ...
            return FindInterfacesOfType(tType, assemblies);
        }

        public IEnumerable<Type> FindInterfacesOfType(Type assignTypeFrom, IEnumerable<Assembly> assemblies)
        {
            return FindTypesOfType(assignTypeFrom, assemblies, true, false, t => t.IsInterface && t != assignTypeFrom);
        }

        private IEnumerable<Type> FindTypesOfType(Type assignTypeFrom, IEnumerable<Assembly> assemblies, bool interfaces, bool abstracts, Func<Type, bool> filter = null)
        {
        ...
        public static ITypeFinder Global => _global;
    }
}
